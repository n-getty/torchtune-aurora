...
# --- 3. Launch training (AFFINITY VARIANT) ---
# Each training rank gets ZE_AFFINITY_MASK=LOCAL_RANK via affinity_launcher.py,
# isolating its L0 contexts from vLLM's tiles. Hypothesis: this fixes the
# persistent step-1 GPU NotPresent segfault seen in colo 10+2 runs.
echo "Starting training on ${TRAIN_TILES} tiles (xpu:0 through xpu:$((TRAIN_TILES-1)))..."
echo "Using affinity_launcher.py (ZE_AFFINITY_MASK=LOCAL_RANK per rank)"
...
python3 -m torch.distributed.run --standalone --nproc_per_node=${TRAIN_TILES} \
    experiments/arena_ipc/affinity_launcher.py \
    --config ${CONFIG} \
