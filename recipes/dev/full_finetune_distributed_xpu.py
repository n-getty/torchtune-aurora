...
#
# XPU-adapted variant of recipes/full_finetune_distributed.py for Intel
# Aurora (Max Series GPU / XCCL).  Mirrors production-validated patterns
# in recipes/dev/grpo_full_finetune_distributed_xpu.py:
# - sys.modules torchtune shim (avoids XCCL USM table corruption)
# - ZE_AFFINITY_MASK detection + per-rank xpu device index
# - device-agnostic empty_cache / memory-history wrappers
# - init_xpu_process_group (strips device_id on multi-node)
# - mpi4py pre-init barrier when CCL_ATL_TRANSPORT=mpi
...
class FullFinetuneRecipeDistributedXPU(FTRecipeInterface):
    """
    XPU-adapted full finetuning recipe for dense transformer-based LLMs (Llama, Qwen, AuroraGPT)
    on Intel Aurora. Mirrors the upstream ``FullFinetuneRecipeDistributed`` body but routes all
    device-specific ops (PG init, memory ops, barrier) through ``torchtune.training`` wrappers
    so the same recipe runs on CUDA or XPU.
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        # which fails for LOCAL_RANK > 0 when device_count==1. Temporarily pin
        # LOCAL_RANK=0 so CheckpointClient's get_device call succeeds. Only
        # apply on the multi-node path (single-tile affinity); on single-node
        # ZE_AFFINITY_MASK is unset and device_count==12, so the wrap is
        # unnecessary AND has been observed to wedge the first FSDP allgather.
...
def recipe_main(cfg: DictConfig) -> None:
    ...
    """
    config.log_config(recipe_name="FullFinetuneRecipeDistributedXPU", cfg=cfg)
    recipe = FullFinetuneRecipeDistributedXPU(cfg=cfg)
