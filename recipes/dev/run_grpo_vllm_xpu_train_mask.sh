...
# --- 3. Launch training (TRAIN-MASK VARIANT) ---
# All training ranks get ZE_AFFINITY_MASK=0,1,...,TRAIN_TILES-1, so each rank
# sees only the training tiles (CCL topology preserved among them) and vLLM's
# tiles 10/11 are invisible. Goal: isolate the persistent step-1 GPU NotPresent
# segfault from L0 cross-tile exposure to vLLM's tiles, without breaking CCL.
echo "Starting training on ${TRAIN_TILES} tiles (xpu:0 through xpu:$((TRAIN_TILES-1)))..."
echo "Using affinity_launcher_train.py (ZE_AFFINITY_MASK=0,1,...,${TRAIN_TILES}-1 for all ranks)"
...
python3 -m torch.distributed.run --standalone --nproc_per_node=${TRAIN_TILES} \
    experiments/arena_ipc/affinity_launcher_train.py \
    --config ${CONFIG} \
