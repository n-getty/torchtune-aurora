...
#PBS -j oe
#
# Aurora GRPO launcher for torchtune XPU recipe.
#
# Usage:
#   Single node:  qsub aurora_grpo.sh
#   Multi-node:   Edit NUM_NODES below, then: qsub aurora_grpo.sh
#   Interactive:  bash aurora_grpo.sh  (from an interactive session)
#
...
RECIPE="dev/grpo_full_finetune_distributed_xpu"
CONFIG="dev/qwen3B_grpo_xpu"
TORCHTUNE_DIR="/lus/flare/projects/ModCon/ngetty/torchtune"
WRAPPER="${TORCHTUNE_DIR}/recipes/dev/aurora_grpo_wrapper.sh"
...
# Optional: production mode (skip non-essential syncs for ~3-8% throughput gain)
# export FSDP_PRODUCTION_MODE=1
...
echo "=== GRPO XPU training complete ==="
