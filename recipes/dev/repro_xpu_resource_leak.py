...
def main():
    ...
    for i in range(args.max_iters):
        ...
        _log_every = int(os.environ.get("REPRO_LOG_EVERY", "5"))
        if rank == 0 and (i + 1) % _log_every == 0:
            elapsed = time.time() - t0
            mem = torch.xpu.memory_allocated(device) / 1e9
            mem_reserved = torch.xpu.memory_reserved(device) / 1e9
            # device-global FREE (mem_get_info) is the discriminator vs ALCF #143:
            # #143 (oneCCL all_gather hidden-temp leak) shows ext_free dropping while
            # torch alloc/reserved stay flat. If THIS FSDP+empty_cache leak instead
            # shows ext_free FLAT right up to the UR40 crash, it is a UR-handle
            # exhaustion distinct from #143's device-global memory leak.
            ...
            print(
                ...
                f"loss={loss.item():.4f} | "
                f"torch alloc/resv={mem:.1f}/{mem_reserved:.1f} GiB | "
                f"ext_free={ext_free:.2f} GiB"
            )
