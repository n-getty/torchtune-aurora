#!/bin/bash
# Per-rank wrapper script for Aurora PPO training.
# Called by aurora_ppo.sh for each MPI rank.
#
# Usage: bash aurora_ppo_wrapper.sh <RECIPE> <CONFIG> [extra tune args...]

set -euo pipefail

RECIPE="${1:?Usage: aurora_ppo_wrapper.sh <RECIPE> <CONFIG>}"
CONFIG="${2:?Usage: aurora_ppo_wrapper.sh <RECIPE> <CONFIG>}"
shift 2
