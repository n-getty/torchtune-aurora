...
| `TORCHTUNE_VARLEN_CACHE_MAX=8` | FIFO cap on the IPEX-varlen no-grad output-buffer caches (`_varlen_out_cache`/`_alibi`/`_seqlens` in `attention_utils.py`). The caches reuse the output buffer per `(b,h,s,d,dtype,device)` for the varlen fast path; on a FIXED-seqlen workload only one key appears, but on VARIABLE-seqlen RL (GSM8K) `s` changes per step so the un-capped dict grew one ~14.7 MiB buffer per distinct shape forever = a ~0.44 GiB/step LIVE-memory leak in the no-grad ref forward (root-caused via the no-FSDP colocate creep bisect, 2026-06-24). FIFO eviction keeps the cap most-recent shapes; within-step + consecutive-same-shape reuse (the speedup) is preserved. Off-path (varlen disabled) unaffected. See `docs/reports/colocate_memory_varlen_and_chunked_vocab_20260624.md`. |
