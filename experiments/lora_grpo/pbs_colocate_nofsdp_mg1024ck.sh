...
#PBS -o /lus/flare/projects/ModCon/ngetty/torchtune/experiments/lora_grpo/logs/pbs_mg1024ck.out
#PBS -e /lus/flare/projects/ModCon/ngetty/torchtune/experiments/lora_grpo/logs/pbs_mg1024ck.err
...
cd "$TT"
NF="$TT/experiments/lora_grpo/logs/pbs_mg1024ck_nodefile.txt"
sort -u "$PBS_NODEFILE" > "$NF"
echo "=== mg1024 job up $(date) | jobid=$PBS_JOBID ==="; cat "$NF"
LOG="$TT/experiments/lora_grpo/logs/pbs_mg1024ck.log"
...
echo "=== maskfree engage/bail + grpo_step path ==="
  grep -oE "grpo_step path: [A-Z_]+|maskfree (engaged|bail[^ ]*|skip[^ ]*)|MASKFREE[^ ]* (ENGAGED|engaged|bail[^ ]*)|grpo_step path: [A-Z_]+" "$RL" 2>/dev/null | head -4
