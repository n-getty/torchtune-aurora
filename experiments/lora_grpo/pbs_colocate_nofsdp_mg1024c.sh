...
#PBS -o /lus/flare/projects/ModCon/ngetty/torchtune/experiments/lora_grpo/logs/pbs_mg1024c.out
#PBS -e /lus/flare/projects/ModCon/ngetty/torchtune/experiments/lora_grpo/logs/pbs_mg1024c.err
...
cd "$TT"
NF="$TT/experiments/lora_grpo/logs/pbs_mg1024c_nodefile.txt"
sort -u "$PBS_NODEFILE" > "$NF"
echo "=== mg1024 job up $(date) | jobid=$PBS_JOBID ==="; cat "$NF"
LOG="$TT/experiments/lora_grpo/logs/pbs_mg1024c.log"
env CONFIG="recipes/configs/dev/production/qwen3_4b_lora_grpo_colocate_xpu.yaml" \
    ...
    NSTEPS=25 NTILES=12 MAX_GEN=1024 VLLM_GPU_MEM=0.15 \
