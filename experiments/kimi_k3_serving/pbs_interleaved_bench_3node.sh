...
#PBS -o /lus/flare/projects/ModCon/ngetty/torchtune/experiments/kimi_k3_serving/logs/interbench.out
#PBS -e /lus/flare/projects/ModCon/ngetty/torchtune/experiments/kimi_k3_serving/logs/interbench.err
...
RUN_DIR=$EXP/logs/interbench_${PBS_JOBID%%.*}
mkdir -p "$RUN_DIR"
...
mpiexec -n 32 -ppn 11 --pmi=pmix \
    ...
    "$PYTHON" "$EXP/bench_interleaved_collectives.py" 2>&1 | tee "$RUN_DIR/bench.log"
