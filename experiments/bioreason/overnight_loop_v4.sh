#!/bin/bash
# Overnight v4 — fixes from v3 lessons:
#  - Re-run configs lost to bad-node x4217c5s1b0n0+x4217c5s2b0n0 (banned:1 PDE crashes after 1-2 steps).
#  - Bug fix: grep -c via "|| echo 0" yields multi-line "0\n0" on grep failure → arithmetic chokes.
#    Use "$(grep -cE ... 2>/dev/null | head -1 || echo 0)" instead.
#  - Bad-node detection: track TRAIN_NODE per iter; if same node crashes with banned:1
#    twice in a row, qdel the hold to force loop onto a different one.
set -euo pipefail
...
RUN_LOG=${STATE_DIR}/loopv4_$(date -u +%Y%m%d_%H%M%S).log
exec >>"${RUN_LOG}" 2>&1
...
log "Overnight loop V4 START (max_runs=${MAX_RUNS}, retries=${MAX_RETRIES_PER_SPEC}); stop=${STOP_FILE}"

# Re-run configs lost in v3 (G=16 long15, G=12 fbs=4, G=16 fbs=4) + remaining
SWEEP=(
    "16,2,1024,15,v4_g16_fbs2_long15"
    "12,4,1024,5,v4_g12_fbs4"
    "16,4,1024,5,v4_g16_fbs4_boundary"
    "24,2,512,5,v4_g24_mg512"
    "32,2,512,5,v4_g32_mg512"
    "16,2,1024,30,v4_g16_stability30"
)
    ...
    while [[ ${RETRY} -lt ${MAX_RETRIES_PER_SPEC} ]]; do
        ...
        while true; do
            ...
            case "${STATE}" in
                R) break ;;
                ""|F|E)
                    log "  ${JOBID} is ${STATE:-GONE}; retry spec"
                    sleep 10
                    continue 2
                    ;;
            ...
            if [[ ${WAIT_SECS} -ge 1800 ]]; then
                log "  Timed out waiting 30min for ${JOBID}; bail spec"
                continue 3
            fi
        done
        log "  ${JOBID} is R"
        ...
        # Single-value extraction (head -1 prevents multi-line "0\n0" arithmetic crash)
        STEPS_OK=$( { grep -cE "Step [0-9]+: loss=" "${LOGFILE}" 2>/dev/null || echo 0; } | head -1)
        OOM_LINES=$( { grep -cE "out of memory|OutOfMemoryError" "${LOGFILE}" 2>/dev/null || echo 0; } | head -1)
        BANNED_LINES=$( { grep -cE "banned: 1" "${LOGFILE}" 2>/dev/null || echo 0; } | head -1)
        TILE_TIMEOUT=$( { grep -c "vLLM tile.*not ready within" "${LOGFILE}" 2>/dev/null || echo 0; } | head -1)
        log "  steps=${STEPS_OK} oom=${OOM_LINES} banned=${BANNED_LINES} tile_to=${TILE_TIMEOUT} elapsed=${ELAPSED}s on ${TRAIN_NODE}"

        # Bad-node detection: 2 consecutive banned:1 on same node = qdel
        if [[ ${BANNED_LINES} -gt 0 ]]; then
            if [[ "${TRAIN_NODE}" == "${PREV_TRAIN_NODE}" && ${PREV_BANNED} -gt 0 ]]; then
                log "  BAD-NODE: ${TRAIN_NODE} crashed banned:1 twice in a row; qdel ${JOBID}"
                qdel "${JOBID}" 2>&1 | sed 's/^/    /' || true
        ...
        # Infra-failure retry: ssh exited <90s with 0 steps and 0 OOM
        if [[ ${ELAPSED} -lt ${MIN_RUNTIME_S} && ${STEPS_OK} -eq 0 && ${OOM_LINES} -eq 0 ]]; then
            log "  INFRA FAIL: ssh exited in ${ELAPSED}s with 0 steps; retry on fresh hold"
            qsub "${HOLD_SCRIPT}" 2>&1 | sed 's/^/    /' || true
            continue
        fi
