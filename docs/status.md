...
Last updated: 2026-08-03 (Qwen3.6-27B HF-wrapper integration plan's Step 4 — the plan's overall pass criterion — PASSED. See entry below.)
...
- **2026-06-21 — SFT no_sync-during-accumulation + ZeRO-2: HW-validated at 2N and rolled into production.** Ported two structural FSDP wins from the BaseMM_PRISM scaling report into the SFT recipes. **(1) no_sync during gradient accumulation** (default-on when `ga>1`): FSDP2 `set_requires_gradient_sync(is_last)` / FSDP1 `no_sync()` suppresses the grad reduce-scatter on all but the last micro-batch, collapsing `ga` cross-rank reductions/step → 1. **(2) ZeRO-2** (`fsdp_reshard_after_forward=false`): keeps full params resident forward→backward, skipping the backward all-gather. **Same-allocation 2N A/B (job 8554260, AGPT-2B gsm8k, ga=4, dp=24, compile+AC):** baseline 21.56 s/step → no_sync 10.01 (**2.15×**) → no_sync+ZeRO-2 8.38 (**2.57×**); tok/s/gpu 91→195→232. **Correctness proven 3 ways:** grad_norm trajectory bit-identical across all cells (same training), CPU grad-equivalence test (grads match to 9.3e-8, 4× fewer reduce-scatters, `test_sft_no_sync_grad_equivalence.py` 7/7), ZeRO-2 ran full 57 steps at 18.9 GiB peak (+4.9 GiB vs ZeRO-3's 14.0). no_sync is free (same numerics/memory); ZeRO-2 trades +4.9 GiB for +16% — flipped only in the validated 2B gsm8k family, opt-in for 4B+. **Applied to both** `full_finetune_distributed_xpu.py` and `lora_finetune_distributed_xpu.py` (LoRA had the same missing-no_sync gap). **Two pre-existing async-dataloader bugs found+fixed along the way:** (a) `pin_memory` default→False — the {compile + pinned forked-worker batches + non-reentrant AC} triple raised a step-0 `CheckpointError` (real, A/B-proven; `compile_dynamic` does NOT fix it); (b) the "2N training divergence" scare was **cosmetic** — `num_workers>0` explodes the *displayed* `running_loss` ~20×/step but grad_norm/fingerprints/num_tokens are all bit-identical to `num_workers=0`, so training is correct (lesson: check grad_norm before crying divergence). Also taught `scripts/check_run_health.sh` the SFT `time_per_step_s` log format (was false-flagging SFT runs DEGRADED; +3 tests, 18/18). BioReason needs nothing — it's GRPO-only and the GRPO base already has no_sync. Memory: `project_sft_no_sync_zero2_2n_validated_20260621`, `project_sft_pinmem_compile_ac_checkpoint_error_20260621`, `project_sft_numworkers_2n_cosmetic_loss_bug_20260621`. **Caveat:** speedups are 2B/gsm8k/seq1024-specific (network-bound small-model step); re-measure for larger models/seq where compute dominates.

- **2026-06-19 — SFT throughput Aurora-vs-Polaris (Qwen3-4B): torchtune-native fixes the broken TRL full-FT cell; corrected the root-cause diagnosis.** Picked up the prior session's TRL `SFTTrainer` handoff (`sft_throughput_aurora_vs_polaris_handoff_20260619.md`), which had a CONFIRMED-broken Aurora full-FT cell (13.59 s/step, MFU 1.7%) blamed on accelerate+xccl host-fallback topology discovery. **Pivoted to the recipe we actually ship** (`recipes/dev/full_finetune_distributed_xpu.py`, `torchrun --standalone`) per the user's "most optimal/stable recipe" goal. **HEADLINE: torchtune full-FT = 4.70 s/step (9,934 t/s), 2.89× faster than TRL on the same 12 tiles**, zero `node_dev_uuids size 1` warnings (XeLink fast path engaged). New report `docs/reports/sft_throughput_aurora_vs_polaris_20260619.md`; new configs `qwen3_4b_sft_throughput_aurora_xpu.yaml` (+ LoRA twin); torchtune launcher `benchmarks/sft_throughput_aurora_vs_polaris/run_aurora_torchtune.sh`; added `time_per_step_s` (engine-agnostic metric) to both XPU recipes' log_dict. **Corrected diagnosis:** the TRL break is the launch path (mpiexec+accelerate host-fallback), NOT `CCL_WORKER_COUNT=4` — re-ran TRL at `=1`, step time unchanged (~14.2 s), topology warning persisted (a hypothesis I led with and the evidence disproved). **Engine delta cuts both ways:** torchtune is 2.89× faster for full-FT but 0.55× (slower) than TRL for LoRA on the same hardware → cross-cluster torchtune-Aurora-vs-TRL-Polaris is engine-confounded (Polaris not yet rerun on torchtune). **torchtune step is compute-bound** at a ~4.2–4.7 s floor (full / LoRA / DDP-LoRA all within 5%); confirmed by AC-off probe OOMing at >50 GiB activations/tile. `compile=True` fails on torch 2.10+xpu (dynamo `AssertionError: 2 != 1`). See `memory/project_sft_throughput_torchtune_vs_trl_aurora_20260619.md` + `feedback_ccl_worker_count_not_fsdp_bottleneck_20260619.md`.

- **2026-06-18 — "Best 2N AGPT-2B GRPO setup" topology study FINALIZED + a wsync regression fixed; torchtune-vs-ezpz 2N data point added.** Answered "given 2 Aurora nodes, where should vLLM live?" across four topologies (sum_digits, G=16, 50 steps; report `docs/reports/agpt2b_2n_topology_throughput_20260618.md`). **HEADLINE: the dominant 2N lever is `data_parallel_replicate_dim=2`, NOT vLLM placement.** Flat colocate dp_rep=1→2 (same 24 tiles, same in-process vLLM, only the FSDP mesh changes) cuts step time **7.8→5.5 s (−29%)** by keeping the per-layer shard collective intra-node (12-way XCCL) instead of cross-node 24-way. **Best 2N setup = flat colocate + `dp_replicate=2`**: fastest (5.5 s/step), most stable (best_acc 1.00), simplest (no separate vLLM node), memory-comfortable (~29/64 GiB). Throughput (steady s/step): colocate-flat 7.8, **colocate-HSDP 5.5**, 11+1-per-node 5.9, dedicated-node 7.6 (wsync-bound, 5.9 s cross-node weight push). Server topologies (11+1, dedicated) are viable but slower/lower-converging on a memory-comfy 2B — they earn their keep only when the model can't colocate or generation dominates. **REAL BUG FIXED (regression, would NaN ALL server runs):** commit `f60efefc` (06-17 wsync refactor) split `_sync_weights_to_vllm_xccl` into per-FSDP-mode helpers but **dropped the broadcast tail for the FSDP1 branch** — `_xccl_gather_fsdp1` gathered weights then returned without sending them, so vLLM generated forever with step-0 weights → frozen-generator drift → ∇logp blowup → NaN by ~step 25. Root-caused by elimination + bisect-by-date (8N HSDP trained fine on 06-16 *before* the refactor; 11+1 first hit it 06-18). Fixed in `weight_sync.py` `_xccl_gather_fsdp1` (greedy CPU batching matching the receiver's `batch_max_numel` contract + `_deferred_broadcast_args`; one mega-batch desyncs the receiver → EngineDeadError, so batch count must match). **Residual "server won't converge" CLOSED as envelope-not-bug:** server NaNs at kl_coeff=0/lr=1e-5 (the ezpz-matched sweep envelope) but trains cleanly at the documented production envelope (GRPOLoss kl_coeff=0.02, lr=5e-6, `always_compute_rollout_logprobs=true`): kl bounded (median 0.06–0.10, 0 NaN), reward 0.16→0.84/0.88 over 50 steps. colocate tolerates kl=0/lr=1e-5; server does not (prod config already halves lr + adds kl_coeff "because 0.0 NaN'd by step 40"). **Two more fixes landed earlier in the same session** (both required to even run the study): (1) **colocate at `dp_replicate>1`** — `_sync_colocated_weights` now supports FSDP1 HYBRID_SHARD via `summon_full_params(rank0_only=False)` + `_fsdp_wrapped_module` prefix strip (was FSDP2-only, crashed at first sync); (2) **2N flat-colocate FSDP2 mesh** — `dp_replicate=1` multi-node left `_dp_mesh=None` → FSDP2 default-mesh mis-enumerated cross-node ranks → `IndexError` at `_init_sharded_param`; now builds an explicit 1-D dp_shard mesh (multi-node only; single-node byte-identical) + registers the world gloo PG for the leak-safe reduce_scatter bounce; (3) **metric-reduce gate** — `log_metrics` world-reduce now gated on `dp_replicate<=1` (was `_shard_pg is None`, which the mesh fix tripped → 2N colocate logged a coarse rank-0-only success metric). **torchtune-vs-ezpz doc:** added the 2N torchtune column (`docs/experiments/torchtune_vs_ezpz.md`) — at 24 ranks torchtune matches/edges ezpz (5e-6 tie 0.594 vs 0.542; 1e-5 0.729 vs ezpz's collapsed 0.083). **Launcher hardening:** `2N_11plus1_sweep.sh` vLLM startup health-retry (Aurora EngineCore flake bit twice); override hooks on `2N_sweep.sh`/`2N_server_sweep.sh`; `_rank_wrapper.sh` WORLD_SIZE PALS fallback (was silently defaulting to 12 → broken 24-rank world). **HPC lessons:** export `PBS_JOBID` *inside* the ssh command (env-prefix doesn't propagate → PALS 17006); a wedged `SSH_AUTH_SOCK` (antigravity agent) hangs the ssh banner — `unset SSH_AUTH_SOCK`. **New CPU tests (full rl suite 466 passed):** `test_xccl_fsdp1_broadcast_present.py` (regression guard for the dropped broadcast), `test_multinode_colocate_dp_mesh.py`, `test_metric_reduce_gate.py`. Memory: `project_agpt2b_2n_topology_throughput_20260618`.

- **2026-06-18 — LoRA-GRPO per-rank COLOCATE mode (in-process TP=1 vLLM per training rank) implemented + validated at 4B; UR:40 crash root-caused; on-XPU base use-after-free fixed.** New `vllm_mode=colocate` path on the standalone LoRA recipe: each rank runs its own in-process vLLM, merges `W_eff` at the trainer, and `load_weights()` into its own engine — no `--enable-lora`, no torch211, frameworks stack. **Headline numbers (4B, 2-node, all measured):** 4B **full-FT colocate OOMs at step 0** (full grad+AdamW + vLLM > tile); **4B LoRA colocate = 7.0 s/step** (on-XPU base, 30/30 clean) or 11.0 s (CPU-base fallback) — **~2.5–4× faster than 4B dense-server (~28 s/step)** at the matched AGPT 16-seq envelope, because colocate generates in-process (~1.6 s vs cross-node 6 s) and LoRA's adapter backward is cheap vs dense's full-4B backward (grpo 17 s) + cross-node wsync. **Pure model-size ratio (matched Qwen3 family, CPU-base): 0.6B 5.0 s → 4B 11.0 s = 6.7× params for 2.2× step time (sub-linear)** — the earlier "4B ≈ 5–10× the 2B" alarm was a confounded server-mode comparison, not a model-size effect. **UR:40 root cause (3 wrong diagnoses, each corrected from data — not summon-leak, not max_num_seqs, not KV-pool):** vLLM KV + FSDP activation buffers grow as rollouts lengthen; XPU can't `empty_cache` (UR-handle guard) so HBM **staircases to banned:1**. It's a one-time sequence-length-driven allocation (phase-probe `COLOCATE_PHASEPROBE` attributed it to gen + grpo_step, sync=0), bounded by **`max_generated_tokens=128`** → flat memory, 30/30 clean. **On-XPU base cache use-after-free FIXED:** `_cache_colocate_base` aliased FSDP's temporary `summon_full_params` buffer (`.to(xpu)` no-op for already-XPU tensors) → PML4 fault at step 2, size-independent (crashed at 0.6B with 50 GiB free); fix = `.clone()`, validated 30/30, wsync 5.4 s → 0.71–0.83 s. **Caveat:** on-XPU free HBM still gently declines (23→12 GiB/30 steps) — fine ≤30 steps; long runs or the paper max_gen=384 need the not-yet-built **warmup-at-max** fix (front-load peak buffers at step 0). **Code (CPU suite green):** colocate path + `_sync_colocated_lora_weights` + `_cache_colocate_base` (LoRA recipe); `validate_vllm_mode`/`tune_lora_name_to_hf` + in-place merge `delta.add_(base_w)` (`lora_helpers.py`); `num_gpu_blocks_override` extended to plain colocate (`vllm_backend.py`). New configs `qwen3_4b_lora_grpo_colocate_xpu.yaml` (+ 0.6B smoke, dense-4B sumdigits), launcher `run_lora_colocate.sh`, tests `test_lora_colocate_{gate,name_translation,merge_equivalence}.py`. **HPC lessons captured:** PALS needs the FULL PBS_JOBID (numeric fails 17006); never pkill mpiexec (kills PALS proxy); PBS snapshots the `.sh` at qsub (recipe/config read at runtime). Report: `docs/reports/lora_colocate_4b_20260618.md`. Memory: `project_overnight_colocate_ur40_plan_20260618`, `project_lora_colocate_implementation_20260618`, `feedback_pkill_mpiexec_wedges_pals_job`, `feedback_pbs_snapshots_script_at_qsub`.
- **2026-06-17 (PM) — LoRA-GRPO code hardening, corrected 4B LoRA-vs-full-FT parity, and a new measurement-validity gate.** A session of LoRA hardening that also produced a hard lesson about trusting runtime numbers. **Code (full RL suite 411 passed / 0 failed):** (1) merged-weight publish **fail-fast** (item C — a failed `/collective_rpc` POST on the default Path A used to warn-and-continue → training silently off-policy on STALE vLLM weights; now raises, gated `lora.fail_on_publish_error` default True; validated under a real rsync-failure this session); (2) config-driven publish timeouts + multi-host tmpfs guard + stale-comment fix in the LoRA recipe; (3) **CHUNKED_BACKWARD non-EP `reduce_scatter` bypass** added to the base `grpo_full_finetune_distributed_xpu.py` (mirrors the SINGLE_BACKWARD bypass; EP byte-identical); (4) opt-in **LoRA surface on the device-agnostic `grpo_full_finetune_distributed.py`** (enables a true LoRA-vs-LoRA CUDA baseline; full-FT path byte-identical) + Polaris config/launchers (run deferred — ALCF Polaris queue blocked by a 480-node job); (5) SFT hardening — 33 new CPU tests, no drift bugs. **Launcher config-drift fixed:** `run_qwen3_4b_lora_2node.sh` now defaults `LORA_USE_RUNTIME=0` (Path A, was the BLOCKED Path B) and `G=8`/`max_gen=384` (validated-safe; the config's paper G=24/max_gen=512 is the documented banned:1 boundary, now flagged in-YAML). **4B baseline (post-hardening):** Path A G=8 → 6/6 clean, ~52–53s/step, 16.35 GiB. **tmpfs publish transport: NET LOSS** (rsync rides bond0 mgmt net not hsn0, <0.11 GB/s; knobs default off). **Corrected LoRA-vs-full-FT 4B parity (the headline correction):** at a matched 16-seq envelope (batch=1, G=16, max_gen=384) **LoRA ~30s/step vs dense ~36s/step — CLOSE, not 5×**; LoRA's real wins are backward **2.6s vs 16–18s (~6–7×)** and memory **~16 vs ~36.6 GiB (~2.3×)**. NOTE Path A LoRA ships the **full merged 6.77 GiB** (same order as dense's 7.49 GiB XCCL sync); the "minimal 66 MB adapter" is Path B only (blocked) — both syncs are async/overlapped. An initial "full-FT 274s, LoRA 5× faster" claim was **WRONG TWICE** (first blamed model cost, then gloo-CPU-bounce reduce_scatter — disproven when the bypass fix didn't move the number); the real cause was a **config artifact** (64-seq/fbs=2/ZeRO-3 → 32 backward chunks × 5.3s). **New durable defense:** `scripts/check_run_health.sh` + `docs/RESULTS_DISCIPLINE.md` + CPU fixture test — a measurement-validity gate that flags degraded runs (gloo-CPU-bounce on non-EP chunked, varlen-skip, banned:1, A/B path mismatch) and sanity-bounds step times vs known baselines; it correctly GREEN-gated the final numbers and would have caught the original mistake. Reports: `docs/reports/lora_vs_fullft_4b_parity_20260617.md` (with full correction history), `lora_status.md` (updated), `run_health_gate_20260617.md`, `chunked_reduce_scatter_bypass_fix_20260617.md`, `lora_grpo_{code,perf}_hardening_20260617.md`, `lora_sft_hardening_20260617.md`, `lora_grpo_cuda_recipe_option1_20260617.md`, `lora_cuda_baseline_plan_20260617.md`. Memory: `project_lora_vs_fullft_4b_parity_20260617`, `project_lora_grpo_4b_baseline_postharden_20260617`, `project_lora_grpo_tmpfs_transport_negative_20260617`.
- **2026-06-17 — Standard step-based checkpoint resume for the GRPO XPU recipe (SHIPPED + validated end-to-end); AGPT-2B 8N growth-past-150 investigated (marginal).** GRPO step-based runs (epochs=1, num_steps=N) are now resumable to continue past their step budget with full optimizer/dataloader/rng/step continuity. Validated 8N HSDP: fresh 150-step run (job 8545903 rc=0) → resume to 300 (8545985 lr=5e-6, 8546049 lr=1e-5), both rc=0, first resumed step=151, momentum continuous (grad_norm 5.6→4.9, no fresh-opt spike), re-resumable checkpoints. **Four resume bugs fixed (each only caught by RUNNING; CPU tests passed throughout):** (1) `intermediate_checkpoint` gate now also true when `save_every_n_steps` set → step-based runs write recipe_state (opt-in; else byte-identical); (2) `STEPS_KEY` persist/restore + setup() derives `_steps_run` on resume (loop continues, not restart); (3) policy/ref checkpointer split (`load_checkpoint(store_as=...)`, ref `resume=False`) → checkpoints in `${output_dir}/epoch_N` not `/ref/`; (4) FSDP1-native optimizer state (`FSDP.optim_state_dict`/`optim_state_dict_to_load`) — the DCP-save/manual-DTensor-load asymmetry crashed `optimizer.step()` on the FSDP1 path; plus don't increment/re-save `_epochs_run` on step-budget stop (was poisoning epochs_run=1 → empty resume loop) + clamp on resume. **Bonus (validated): resume at a different lr / constant lr** — config lr re-applied after the optimizer load (which restores the checkpoint's saved lr), and `lr_scheduler=null` tolerated. Usage: `RESUME_FROM=<output_dir>/epoch_0`; lr change via `EXTRA_OVERRIDES="optimizer.lr=1e-5 lr_scheduler=null"`. **Growth-past-150 (the science question): MARGINAL at 8N.** Late-window (250-300): lr=5e-6 resume 0.179/0.069 (plateau, drifts down), lr=1e-5 resume **0.195/0.091** (~+10% rel over the 100-150 baseline of 0.188/0.083, but within single-seed noise — NOT the 2N plateau-break of +2.4pp). KL bounded (0.15-0.29), so genuine convergence. The 8N run sees 14 distinct prompts/step → 150 8N-steps ≈ 2100 2N-prompt-exposures → converges faster/flatter, hitting the same ~8-9% GSM8K capability ceiling that capped 2N. **Conclusion: pushing past ~9% needs a capability lever (SFT init / reward / base model), not more GRPO steps.** Resume makes long continuations cheap to try. New: `docs/reports/agpt2b_8n_hsdp_resume_and_growth_20260617.md`, memory `feedback_grpo_step_based_resume.md`. CPU tests `test_checkpoint_resume_state.py` (14).

- **2026-06-16 — AGPT-2B GRPO 8-node HSDP scale-up SHIPPED — ~10× distinct-prompt throughput AND +26% reward / +53% success vs 2N at equal steps.** Scaled the AGPT-2B GRPO path from the old "2N" topology (1 *single-node* FSDP replica + 1 vLLM node, `batch_size=1` → **1 distinct prompt/step**, DP=1, high-variance gradient) to the standard disaggregated data-parallel topology: **7 training replicas (`dp_replicate=7 × dp_shard=12`, dense Llama3 FSDP1 HYBRID_SHARD within node, grads all-reduced across replicas) + 1 dedicated 12-server vLLM pool** shared by all replicas. Each replica's shard-leader pulls its OWN prompt slice, generates against the shared pool, and broadcasts completions node-locally over the gloo dp_shard PG → **14 distinct prompts/step**. Weight sync: only global rank 0 gathers + broadcasts to the pool each step (Llama Q/K un-permute applied). **HEADLINE A/B (both wsync ON, G=8, on-policy ratios=1.0; 8N job 8545190 150/150 rc=0 vs 2N job 8544461):** distinct-prompt throughput **0.85/s vs 0.082/s (~10×)**; late-window (steps 100-150) mean reward **0.172 vs 0.137 (+26%)**, success **0.067 vs 0.044 (+53%)** — the lower-variance gradient from 14 distinct prompts/step is the mechanism (single-seed each; replica pending). Per-step 16.4s vs 12.25s (+34%), decomposed as +4s grpo (fbs=4 → 2× backward chunks + cross-node gloo collectives) and +1.5s gen (7 replicas share one vLLM node), partly offset by faster wsync (0.6s vs 1.9s); the +34% is the topology cost at the memory-safe envelope and is NOT recoverable via fbs (fbs=8 re-triggers fragmentation, job 8545370). Uses all 12 tiles/node (vs 2N's 11; tile-11 oneDNN is 32B-specific). **Validated envelope (TIGHT): G=8, batch_size=2, fbs=4, ref_fbs=4, gc:0.8** — per-rank buffer volume = bs×G = 16 seqs/rank; any of {bs=4, G=16, fbs=8} pushes the unlucky replica's PyTorch reserved pool to the 64 GiB tile → banned:1 at step ~12 (the endurance bug, root-caused via mem-probe to reserved-pool *fragmentation*, NOT a CCL/L0/OFI-MR leak — torch_resv climbs/spreads while torch_alloc & external stay flat; two wrong guesses, inter-node all_reduce + OFI-MR, were disproven by the probe). **Diagnostic lesson: with banned:1, read the mem-probe before hypothesizing a transport/leak fix.** Recipe changes (all `dp_replicate==1`-guarded → 2N path byte-identical): `_setup_data` HSDP-aware sampler, `_generate_with_vllm`/`_broadcast_query_responses` per-replica shard-leader gen + gloo node-local broadcast, vLLM clients on every shard-leader, `_setup_model_fsdp1_hsdp` sets `_fsdp2_param_groups_meta`, `distributed.py` `enable_fsdp1_hsdp_inter_node_gloo`. The XCCL FSDP1 server wsync path already handled HSDP (rank-0-gated, Q/K un-permute) — just enabled in config + re-added the vLLM `--worker-extension-cls`. New: config `auroragpt_2b_grpo_7n_gsm8k_hsdp_xpu.yaml`, launcher `experiments/auroragpt_2b_bakeoff/run_agpt2b_7n_hsdp_8node.sh` + wrapper (qsub-only — mpiexec --pmi=pmix needs the PBS process tree; FQDN hostfile no `:N`; node-count-flexible select=N→N-1 replicas+1 vLLM), tests `test_hsdp_server_sampler_contract.py` + `test_fsdp1_hsdp_inter_node_gloo_patch.py`, report `docs/reports/agpt2b_8n_hsdp_scaleup_20260616.md`, memory `project_agpt2b_7n_hsdp_launcher.md`. Follow-ups: 2nd seed; longer run (reward still climbing at step 150); 2nd vLLM node for gen contention; per-replica async.
- **2026-06-16 — AGPT-2B mathmix→GRPO: replica confirms the lift (n=2), and a longer + higher-LR run breaks the ~6.5% success plateau.** Two 2N debug runs (both rc=0) closed the open items from the 2026-06-15 mathmix result. (1) **Replica (job 8544461)** reproduces the n=1 mathmix→GRPO result to within ~1% absolute on every metric — reward all 0.1754 vs 0.1720, late 0.1370 vs 0.1400, success 6.25% vs 6.75%, num_stop 15.72/16 — well inside the 4–7% SFT variance band. n=2 averages: reward 0.1737 / late 0.1385 / success 6.50%. **Mathmix is the shipped AGPT-2B GRPO baseline; bakeoff launcher default `MODEL_PATH` updated to mathmix epoch_2.** (2) **Long+lr run (job 8544526)**: 225 steps @ lr=1e-5 (vs validated 150 @ 5e-6) shows the plateau is a **step-count ceiling, not a wall** — first-150 ties baseline (6.54% vs 6.50%), but the extended window 150–225 climbs to 7.83% and the tail 175–225 reaches **8.88% success (+2.4pp / ~37% rel), reward still rising at step 225**. lr=1e-5 matched 5e-6 over the first 150 with grad_norm controlled (mean ~12, no 200+ spikes — `clip_grad_norm=1.0` holds it) and KL bounded (settles to 0.06–0.23 band) — the YAML's "halved to 5e-6 for stability" caution is over-conservative given the clip. **Correction:** the cosine LR schedule is effectively flat at these run lengths (denominator = `total_epochs×steps_per_epoch` = 7473, so lr decays only ~0.1% over 225 steps) — the earlier "late-phase dip = cosine artifact" framing was wrong; annealing is not in play. Steps and LR are confounded in run 2 by design (one aggressive run inside the 60-min debug cap); a 225@5e-6 control to disambiguate is deferred. New: `docs/reports/agpt2b_mathmix_grpo_replica_and_plateau_20260616.md`, launcher `experiments/agpt2b_sft/pbs_2n_grpo_mathmix_long_lr1e5.sh`, memory `project_agpt2b_mathmix_grpo_long_lr1e5_20260616.md`.
- **2026-06-15 — AGPT-2B multi-corpus mathmix SFT → GRPO: success rate moves for the first time (2.79% → 6.75%, +142%).** Predecessor (2026-06-14, GSM8K-only SFT) lifted GRPO mean reward +28% over raw init but success rate stayed flat at ~2.8% because the `instruct_dataset` SFT path never emitted `<answer>...</answer>` tags and the strict `FormattedMathCorrectnessReward` could not fire. New loader `torchtune/dev/sft/auroragpt_math_mix.py` builds a `ConcatDataset` of GSM8K (×3 replicas) + MATH + MetaMathQA-25K (~60K rows/epoch), all rendered into the exact `PREAMBLE_PROMPT + <think>/<answer>` template the GRPO reward parser expects. 3-ep SFT @ lr=1e-5 (2N, 2h50min wall, loss 7.03→0.58) feeds a 150-step GRPO run that lifts mean reward 0.080→0.172 (+116%), success rate 2.79%→6.75% (+142%), `num_stop_tokens` 15.68/16 (98%), response_len 511→155. n=1 vs n=2 baselines; SFT-side variance band was 4–7% relative on the predecessor and the lift is far outside that, but the formal replica is the next step. Two carry-forward lessons: (a) `compile: true` is incompatible with variable-length corpora (exhausts dynamo recompile_limit (8) in the first epoch — the GSM8K-only +12-23% compile win required GSM8K's tight shape envelope); (b) dataset loader must hard-cap at `tokenizer.max_seq_len` (truncate preamble first), because smoke runs over a small sample miss the corpus long tail. Code: 2 XPU SFT recipes (`recipes/dev/{full,lora}_finetune_distributed_xpu.py`), `AuroraGPTTokenizer.tokenize_messages` + `__call__`, mathmix loader, 15 AGPT-2B configs, bakeoff launcher default fix (`${VAR:-default}` form), 3 CPU regression tests. Docs: `docs/features/agpt2b_sft_pipeline.md` (primer) + `docs/experiments/agpt2b_sft_to_grpo.md` (ablation). Commit `57f9596f`.
- **2026-06-13 — AGPT-2B 2N GSM8K production exemplar fully validated end-to-end.** First AGPT-2B GRPO production reference on the codebase. Three validation runs (all exit=0): (1) job `8540864` ran 150/150 steps clean on the **pre-Stage-1** config — proves the chunked-backward path clears the documented sig#2 step-62 L0 wall; (2) job `8541808` Stage-1 50-step bisect proves the new vLLM `stop_strings` + EOS-injection fix in `_call_vllm_http` finally surfaces a real learning signal — `num_stop_tokens` 0→14.7 of 16, `response_lengths` 511→111; (3) **job `8541918` 150-step production run on the renamed launcher with the Stage-1 fix completed clean: 150/150 steps, `num_stop_tokens > 0` on every step, mean 14.44/16, `response_lengths` mean 121, mem flat 24/58.8 GiB, no NaN, ~33 min wall (~13 s/step), exit=0**. Reward is non-degenerate but does not strongly trend up in 150 steps — the algorithm/reward tuning phase is next (Stages 2-4 in the report). Before the fix, vLLM had no way to know to stop, AGPT-2B (raw pretraining ckpt, never naturally emits EOS) ran every completion to `max_tokens=511`, and the loss was minimizing noise. Fix landed in `torchtune/dev/rl/vllm_client.py` (new `stop` + `stop_token_ids` params with `include_stop_str_in_output=true`), `recipes/dev/grpo_full_finetune_distributed_xpu.py` (forwards `cfg.stop_strings`, injects EOS at completion boundary so `truncate_sequence_at_first_stop_token` finds it), and the YAML (new `stop_strings: ["</answer>", "User:", "\nUser:"]`). Files renamed: `pbs_2n_gsm8k_server_stable.sh` → `pbs_2n_gsm8k_production.sh`, YAML drops the `_stable` suffix; older `pbs_2n_gsm8k.sh` (colocate) moved to `archive/`. New artifacts: `experiments/auroragpt_2b_bakeoff/smoke_2n_gsm8k.sh` (5-step regression, ~10 min), `docs/reports/agpt2b_2n_gsm8k_production_20260613.md`, memory `project_agpt2b_2n_gsm8k_production_envelope.md`. CPU regression test `tests/torchtune/dev/rl/test_vllm_client_stop_tokens.py` (11 tests, all pass). Open levers for reward tuning (Stage 2: `always_compute_rollout_logprobs`; Stage 3: bump formatting reward; Stage 4: extend `max_generated_tokens`) documented in the report.
- **2026-06-11 — "ezpz convergence gap" closed: cross-framework lr sweep shows no algorithmic gap.** Definitive cross-framework characterization on AGPT-2B sum_digits (50 steps, single seed per cell). torchtune-XPU 1N colocate vs ezpz/TRL 2N HF `.generate()`, identical task/B/G/T/max_gen, varied lr ∈ {1e-7, 5e-7, 1e-6, 5e-6, 1e-5, 5e-5}. **Both stacks tie at lr=5e-6 (best_acc=0.542 each).** At lr=1e-5 torchtune climbs to 0.667 while ezpz's `grad_norm` collapses to 0 (stuck at 0.083) — torchtune has higher lr headroom, likely from bf16 grad path vs HF Trainer's fp16 grad scaler. Both diverge symmetrically at lr=5e-5. **TRL default `learning_rate=1e-6` is the dead zone for both stacks** — single-seed accuracy bounces 0.08-0.25 (noise floor). All prior bake-off "ezpz wins" results were measured at this dead-zone lr where 30-50% deltas are statistical noise. **Implication: stop comparing frameworks at "matched" lr — always sweep lr per stack first.** Update bake-off launcher defaults from `TT_LR=1e-6` → `5e-6` or `1e-5`. Side findings: (a) ezpz on 1 node crashes at step 8 with UR:40 (HF Trainer per-step `torch.xpu.empty_cache()` exhausts L0 event pool); only 2N is testable. (b) Antigravity walkthrough proposed `T=1.0` substitution in `batched_logits_to_logprobs` and mask-sign inversion in `response_padding_masks` — both contradict TRL canon and the mask-inversion ZEROES the chunked-path gradient (mistakes `True=padding` for `True=valid`). Reverted to clean `c58acbed`. (c) Phase 4 IS-correction `logprobs_mode="processed_logprobs"` + DAPO loss aggregation work was real and TRL-correct but **not load-bearing** for the bake-off — patches were reverted by walkthrough application; re-land when needed. Memory: `project_lr_sweep_torchtune_vs_ezpz.md`, `project_ezpz_gap_was_lr_not_code.md`, `project_vllm_is_correction_phase4_results.md`. Sweep logs at `experiments/auroragpt_2b_bakeoff/logs/lr_sweep_sum_digits_20260611_175158/` (torchtune) + `lr_sweep_ezpz_2n_sum_digits_20260611_194008/` + `..._194740/` (ezpz 2N).
...
- **Production-ready paths**:
  ...
  - **LoRA-GRPO 4B/8B 2-node (UPDATED 2026-05-05)**: two publish paths kept available; choose per workload, do not abandon either.
    ...
    - **Colocate (NEW 2026-06-18) — in-process TP=1 vLLM per rank** (`vllm_mode=colocate`): each rank merges `W_eff` and `load_weights()` into its own engine, no `--enable-lora`/torch211. 4B validated **7.0 s/step** (on-XPU base merge, 30/30 clean; CPU-base fallback 11 s) — **~2.5–4× faster than dense-server (~28 s)** at the AGPT 16-seq envelope, and fits where full-FT colocate OOMs. **Requires `max_generated_tokens=128`** (the validated-safe ceiling): vLLM/FSDP buffers grow with rollout length and XPU can't `empty_cache` → HBM staircases to banned:1 at longer gen. Pure size ratio 0.6B→4B = 5.0→11.0 s (6.7× params, 2.2× step). Caveat: on-XPU free HBM gently declines (fine ≤30 steps; long runs need the not-yet-built warmup-at-max). See `docs/reports/lora_colocate_4b_20260618.md`.
