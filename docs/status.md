...
Last updated: 2026-04-30 (BioReason 2-node XCCL wsync end-to-end VALIDATED — backbone-prefix bug fixed in `_sync_weights_to_vllm_xccl`; `WSYNC_TOPOLOGY=node_fanout` real 2-hop validated 10/10 clean at G=4 and 5/5 clean at G=8 + IPEX varlen on hold 8462930. See `docs/reports/bioreason_xccl_2hop_validated_20260430.md`. Earlier: Qwen3-30B-A3B EP=4/DP=6 v8 series first `loss=` line at G=1/fbs=1/NSTEPS=1 on 3-node hold; v8m_b + v8n diagnose unified architectural blocker — v59 `reduce_grads=False` keeps ~18.5 GiB FSDP2 unsharded grads alive across BOTH chunk and step boundaries, blocking G≥2 and NSTEPS≥2 alike)
